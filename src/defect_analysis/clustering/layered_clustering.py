...
def extract_stitching_features(tiles: List[MetaAtom]) -> np.ndarray:
    ...
    for tile in tiles:
        ...
        # Row/column profiles and jump locations
        ...
        row_jump_idx = np.argmax(row_grad) if len(row_grad) > 0 else 0
        col_jump_idx = np.argmax(col_grad) if len(col_grad) > 0 else 0
        
        max_row_jump = row_grad[row_jump_idx] if len(row_grad) > 0 else 0
        max_col_jump = col_grad[col_jump_idx] if len(col_grad) > 0 else 0
        max_jump = max(max_row_jump, max_col_jump)
        
        # Check intersection: Does the max jump line hit a feature (localized variance)?
        hits_blob = 0.0
        # Check standard deviation along the jump line. 
        # Background stitches are flat; hitting a pillar creates high variance.
        std_thresh = (np.abs(img.max() - img.min()) * 0.1)
        
        if max_row_jump > max_col_jump:
            # Row jump (horizontal line)
            # Check variance across the jump row
            if np.std(img[row_jump_idx, :]) > std_thresh:
                hits_blob = 1.0
        else:
            # Col jump (vertical line)
            if np.any(np.std(img[:, col_jump_idx])) > std_thresh:
                hits_blob = 1.0
        ...
        stitching_score = (max_jump + max_split + edge_center_diff) * (0.5 + 0.5 * hits_blob)
        tile.features['stitching_score'] = stitching_score
        tile.features['max_intensity_jump'] = max_jump
        tile.features['hits_blob'] = hits_blob
        
        features.append([
            max_row_jump, max_col_jump, max_jump,
            vertical_split, horizontal_split, max_split,
            edge_center_diff, max_horiz_edge, max_vert_edge, 
            stitching_score, hits_blob
        ])
...
def run_layered_pipeline(tiles: List[MetaAtom], verbose: bool = True) -> Dict[str, ClusterResult]:
    ...
    # Post-process Stitching with consensus threshold (>= 5 tiles in row/column)
    # Post-process Stitching with consensus threshold (>= 15 tiles in row/column)
    detected_indices = set(result3.extracted_indices)
    rows_with_hits = {}
    cols_with_hits = {}
    
    mean_score = 0
    if len(detected_indices) > 0:
        mean_score = np.mean([current_tiles[i].features['stitching_score'] for i in detected_indices])
        if verbose:
            print(f"  Stitching Candidate Mean Score: {mean_score:.2f}")
            print(f"  Row hit counts: {[(r, len(idxs)) for r, idxs in rows_with_hits.items() if len(idxs) > 5]}")
            print(f"  Col hit counts: {[(c, len(idxs)) for c, idxs in cols_with_hits.items() if len(idxs) > 5]}")
    
    final_stitching_indices = set()
    final_irregular_indices = set()

    # Noise floor gating: If the cluster isn't significantly "shouty", it's just noise
    if mean_score < 25.0 and len(detected_indices) > 0:
        if verbose:
            print("  [Gating] Stitching candidate scores too low; treating as noise/irregular.")
        # Re-classify all detections as irregular and skip systemic flagging
        final_irregular_indices = detected_indices
    elif len(detected_indices) > 0:
        systemic_indices = set() # Indices that belong to a systemic row/col
        
        # Identify systemic rows/columns
        for r, idxs in rows_with_hits.items():
            if len(idxs) >= 15:
                # Entire row is flagged as stitching
        ...
        for c, idxs in cols_with_hits.items():
            if len(idxs) >= 15:
                # Entire column is flagged as stitching
                for i, t in enumerate(current_tiles):
                    if t.col == c:
                        final_stitching_indices.add(i)
                        systemic_indices.add(i)
                        
        # Isolated hits that are NOT in a systemic row/col become 'Irregular'
    ...
    # Unified 'remaining' excludes both 'Stitching' and 'Irregular'
    excluded = final_stitching_indices.union(final_irregular_indices)
    remaining_indices_mask = [i for i in range(len(current_tiles)) if i not in excluded]
    
    current_tiles = [current_tiles[i] for i in remaining_indices_mask]
    current_indices = [current_indices[i] for i in remaining_indices_mask]
    
    if verbose:
        print(f"  Systemic Stitching: {len(final_stitching_indices)} | False Positives -> Irregular: {len(final_irregular_indices)} | Remaining: {len(current_tiles)}")
